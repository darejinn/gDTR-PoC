%%%%%%%% MANUSCRIPT VERSION (general single-column journal/preprint format)
%% gDTR: Layer-wise Settling Depth as an Interpretability Axis
%% for Genomic Causal Foundation Models

\documentclass[12pt]{article}

% --- Page layout: 1-inch margins, US letter ------------------------
\usepackage[letterpaper, margin=1in]{geometry}

% --- Typography (compile this file with XeLaTeX or LuaLaTeX:
%     `xelatex gdtr_paper_manuscript_plaintext.tex` --- so that Unicode
%     subscripts/Greek letters in the table cells render natively.) ----
...
\fi\fi
\usepackage{setspace}
\onehalfspacing       % 1.5 line spacing (manuscript convention)
...
% --- Bibliography (numeric citations) -----------------------------
...
% --- ICML stub commands so the rest of the source still compiles ---
% (we removed the icml2026 package; these stubs no-op the leftover
%  \icml... markup. \twocolumn[...] is also handled below.)
...
% Stub the ICML \icmltitle so the rest of the source can stay verbatim.
\newcommand{\icmltitle}[1]{\title{#1}\maketitle}

% In single-column manuscripts, \begin{figure*}/{table*} should behave
% like normal figure/table environments.
...
% Running title (kept for ICML compat — no-op in manuscript).
\icmltitlerunning{gDTR: Layer-wise Settling Depth for Genomic Causal Foundation Models}

\title{\textsc{gDTR}: Layer-wise Settling Depth as an Interpretability
       Axis for Genomic Causal Foundation Models}
...
\texttt{data\_ptr}(h₃₀) vs.\ h₃₁ & distinct &
   physically separate copies \\
cos(h₃₁, h\_norm) & 0.6855 &
   post-norm differs from raw h₃₁ \\
cos(h₃₀, h\_norm) & 0.6855 &
   identical to h₃₁ \\
cos(h₂₉, h\_norm) & -0.013 &
   deepest distinct tap (L*=29) \\
\bottomrule
\end{tabular}
\end{table}
...
donor/acceptor positions converge ~ 3 layers earlier than the
intronic baseline.}
\label{tab:splice}
\centering
\small
\begin{tabular}{@{}lccc@{}}
\toprule
Context & mean c (chr22) & mean c (chr17) & n (chr22) \\
\midrule
...
\midrule
RepeatMasker low-complexity   & 2.02× & ≈ 0 \\
GENCODE 5' UTR              & 1.95× & ≈ 0 \\
ENCODE cCRE-ELS (enhancer)    & 1.90× & < 10⁻³⁰⁰ \\
GTEx eQTL (4-tissue union)    & 1.62× & 5.4× 10⁻⁵⁶ \\
RepeatMasker simple\_repeat   & 1.52× & ≈ 0 \\
GWAS Catalog v1.0 SNPs        & 1.50× & 1.6× 10⁻⁷ \\
RepeatMasker LTR class        & 1.39× & ≈ 0 \\
RepeatMasker LINE class       & 1.31× & ≈ 0 \\
ENCODE cCRE (any)             & 1.28× & ≈ 0 \\
\bottomrule
\end{tabular}
\end{table}
...
max|ΔD₍cos₎| to its closest benign neighbour's.}
\label{tab:variant-classes}
\centering
\small
\begin{tabular}{@{}llccc@{}}
\toprule
Variant & Class & argmax layer & max|ΔD₍cos₎| & matched B/LB ratio \\
\midrule
\emph{BRCA1} 17:43076602 G→T (canonical splice region) &
   P/LP (3-star) & ℓ=7 &
   3.90× 10⁻² & 11.1× \\
\emph{TP53} 17:7674220 C→T (p.R175H, missense) &
   P/LP (3-star) & ℓ=28 &
   2.06× 10⁻² & 8.0× \\
\emph{BRCA1} 17:43057063 G→A (frameshift locus) &
   P/LP (3-star) & ℓ=24 &
   3.67× 10⁻² & 1.5× \\
\bottomrule
\end{tabular}
...
\midrule
Best single-layer ΔD₍cos₎ (ℓ=30 tap) & 1
   & 0.729 [0.717, 0.741]
   & 0.726 [0.694, 0.758] \\
Best single-layer ΔD₍jsd₎ (ℓ=29 canonical tap) & 1
   & 0.794 [0.781, 0.806]
   & 0.787 [0.752, 0.821] \\
Evo~2 log-likelihood & 1
   & 0.751 [0.738, 0.764]
   & 0.793 [0.740, 0.846] \\
32-d ΔD₍jsd₎ vector & 32
   & 0.823 [0.813, 0.832]
   & 0.821 [0.790, 0.853] \\
\textbf{32-d ΔD₍cos₎ vector (primary)} & 32
   & 0.844 [0.831, 0.857]
   & 0.843 [0.811, 0.876] \\
...
\begin{table*}[t]
\caption{Pairwise Spearman ρ of per-window mean settling depth
...
% bibstyle is set in the preamble (unsrtnat).
...
\begin{table}[h]
\caption{Cohen's d sweep over (γ₍cos₎, ρ). The locked
...
\toprule
γ₍cos₎ ρ
   & ρ=0.70 & ρ=0.75 & ρ=0.80 & ρ=0.85 & ρ=0.90 \\
\midrule
...
\midrule
ℓ=2 (\texttt{hcl}) & 1,259 & 5.6 & 0.9956 \\
ℓ=12 (\texttt{hcm}) & 742 & 13.6 & 0.9816 (worst) \\
ℓ=22 (\texttt{hcm}) & 418 & 0.41 & 0.9990 \\
ℓ=28 (\texttt{hcs}) & 822 & 0.34 & 0.9996 (best below tap) \\
ℓ=29 (canonical tap, \texttt{hcm}) & 510 & 0.20 & 0.9996 \\
\bottomrule
\end{tabular}
\end{table}
...
depth-normalised scales (~ 3 layers below intron in 32-layer
Evo~2; ~ 0.34 layers below intron in 8-layer HyenaDNA). MLM
...
\toprule
Model & Kind & L & γ\_q₇₀
   & mean c (intron) & mean c (coding exon)
   & mean c (donor / acceptor) \\
\midrule
...
discriminative mass at the canonical tap ℓ=29; cosine spreads it
...
\begin{tabular}{@{}llcc@{}}
\toprule
Layer & Block type & ΔD₍jsd₎ AUROC & ΔD₍cos₎ AUROC \\
\midrule
0  & embed                    & 0.605 & 0.519 \\
...
runtime, and per-model q₇₀-calibrated γ₍cos₎ for the four
...
HyenaDNA-large-1m & Pure Hyena                          & 8 & 256
   & 6,001 (1\,bp + BOS)   & ~ 4\,min    & 0.358 \\
NT-v2~500M        & Transformer MLM (k-mer)           & 29 & 1024
   & 671 (k=6, ~ 4\,kb) & ~ 7.5\,min  & 0.533 \\
DNABERT-2~117M    & Transformer MLM (BPE)               & 12 & 768
   & ~ 600 (BPE, ~ 3\,kb)        & ~ 3\,min    & 0.677 \\
\bottomrule
\end{tabular}
\end{table}
