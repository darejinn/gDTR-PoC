...
For variants, we use the layer at which $|\DDcos|$ peaks as a depth probe: it asks where the residual trajectory changes most between reference and alternate sequence. This is not a clinical scorer claim. We test this on a ClinVar~\cite{landrum2020clinvar} 15-cancer-gene cohort with cached SNV features and a newly forwarded indel panel. Variants are class-stratified using ClinVar \texttt{MC} priorities (canonical-splice $\succ$ nonsense $\succ$ frameshift $\succ$ missense $\succ$ synonymous $\succ$ intron).
