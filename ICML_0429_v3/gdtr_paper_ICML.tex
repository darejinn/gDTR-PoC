...
% Map \sffamily to Helvetica (phv) so the TikZ schema in Fig 1(a) visually
% matches the matplotlib raster in Fig 1(b), both rendering as a grotesk
% sans-serif rather than the Computer-Modern default.
...
\begin{figure*}[!t]
\centering
\resizebox{0.96\textwidth}{!}{\input{figures/fig1_schema.tex}}
\vspace{0.35em}
\includegraphics[width=0.64\textwidth]{figures/fig1_trajectory.png}
\caption{\gDTR{} measures the layer at which a token's representation stabilises. \textbf{(a)}~Pipeline: each Evo~2 layer emits a residual-stream tap $h_\ell(t)$; the cosine lens compares that tap to the final-layer state $h_L(t)$; a running-minimum envelope tracks the best match so far; and the first threshold crossing defines the settling depth $c(t)$. Lower $c(t)$ means earlier stabilisation. \textbf{(b)}~Two chr22 trajectories illustrate the readout: a splice-donor token settles at $\ell\!=\!22$, whereas an intronic token settles at $\ell\!=\!31$.}
\label{fig:schematic}
\end{figure*}
...
\begin{equation}
  ...
  \label{eq:settling}
\end{equation}
with $\runmin\,\Dcos(\ell,t) = \min_{k\le\ell}\Dcos(k,t)$. Lower $c(t)$ means earlier stabilisation; higher $c(t)$ means that the token remains unresolved until later layers. NLP transformers exhibit near-monotonic logit-lens convergence~\cite{nostalgebraist2020logitlens,belrose2023tunedlens}; genomic CLMs do not (Hyena alignment rotations, Evo~2 idle final block), so the running-min collapses non-monotonicity into a monotone-non-increasing trace. The threshold $\gamma_{\cos}$ is set by \emph{regional $q_{70}$ calibration}: the 70th percentile of the running-min at the penultimate layer over the analysis region. The operating point sits inside a flat $5\!\times\!5$ grid sweep over $(\gamma_{\cos},\rho)$ (App.~\ref{app:gamma-sweep}).

\paragraph{Evo 2's idle final block.} On chr22 sanity sequences, Evo~2's last attention block is architecturally idle: $\max_{t}|h_{30}(t)\!-\!h_{31}(t)|\!=\!0$ exactly (block~31 is a residual passthrough), while $\cos(h_{29},\hnorm)\!=\!-0.013$ versus $\cos(h_{30},\hnorm)\!=\!\cos(h_{31},\hnorm)\!=\!+0.6855$, identifying block~30 as the rotation that aligns the representation with the post-final-norm direction and block~29 as the deepest tap that is still interpretively distinct from $\hnorm$. The canonical NLP convention of ``tap the last block'' therefore breaks; we lock the canonical tap at $L^\star\!=\!29$ and use $\hnorm$ as the reference (full evidence in App.~\ref{app:architectural-quirk}). This handling is itself a method contribution for subsequent Evo~2 interpretability studies.
...
\textbf{Context-level ordering.} On canonical genomic contexts (Fig.~\ref{fig:shallowness}a), splice donor and acceptor sites settle $\sim 2$ layers earlier than the intronic baseline, while most non-splice contexts cluster near or above the baseline. We report Cohen's $d$ as the primary effect-size measure throughout; Mann--Whitney $p$-values reach $<\!10^{-4}$ at all contexts but are inflated by the very large $n$ ($\sim\!10^7$ pooled positions) and should be read as a sanity rather than a magnitude statement. The clearest non-splice extension is ENCODE cCRE-ELS enhancer-like elements~\cite{moore2020encode}: on chr22, cCRE-ELS positions are shallower than a per-position background by Cohen's $d\!=\!-0.190$, against a splice-donor reference of $-0.433$ (Fig.~\ref{fig:shallowness}b). GTEx eQTLs~\cite{gtex2020} ($d\!=\!-0.022$) and GWAS Catalog SNPs~\cite{sollis2023gwas} ($d\!=\!-0.040$) point in the same direction but have biologically marginal effect sizes. We therefore treat splice and cCRE-ELS as the main positive signals, and report 5$'$~UTR as anomalous because it is partly entropy-confounded (\S\ref{sec:discussion}).

\textbf{Entropy does not explain the splice signal.} A natural concern is that $c(t)$ merely tracks the per-position next-token Shannon entropy $H_t$ of the model. On a control panel of $120$ random chr22 windows ($720{,}000$ positions) we compute $H_t$ from the post-norm logits and find an overall Spearman $\rho(c, H_t)\!=\!-0.079$ (per-context $|\rho|\!\le\!0.15$ except $5'$~UTR; \S\ref{sec:discussion}). On the same chr22 panel the splice-donor-versus-intron Cohen's $d$ is $-0.452$; after regressing $c$ on $H_t$ and re-measuring on the residual it strengthens to $-0.583$. Next-token entropy therefore does not explain the splice depth ordering.

\subsection{Motif edits separate detection from flanking context}
\label{sec:motif-controls}

Settling depth is not simply a meter of motif intrinsic strength. We dissociate central motif detection from flanking-context integration on $1{,}000$ canonical GT-AG donors (chr22, $\pm 10$~bp pad-averaged $c$). Real donors settle at $\bar c\!=\!26.77$. (i) Replacing the central GT with AA, while preserving the entire $\pm 100$~bp flank, deepens $\bar c$ by $0.46$ layers (Cohen's $d\!=\!-0.09$, paired Wilcoxon $p\!=\!2.3\!\times\!10^{-32}$), consistent with a small but reliable signal carried by the canonical dinucleotide. (ii) Dinucleotide-shuffling the $\pm 100$~bp flank while preserving the central GT \emph{shallows} $\bar c$ by $3.18$ layers ($d\!=\!+0.51$, $p\!=\!4.1\!\times\!10^{-59}$): the isolated GT becomes easier to stabilise, whereas the real donor context requires deeper flanking-grammar integration. The within-splice motif breakdown in App.~\ref{app:splice-canonical} follows the same cautionary logic: rare or non-canonical motifs can settle earlier than canonical GC-AG, so $c(t)$ should be interpreted as detection plus context integration, not as motif strength alone.

\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_shallowness.png}
\caption{Splice and enhancer annotations shift toward shallower settling. \textbf{(a)} Per-context mean settling depth $\bar c$ pooled over chr17 (validation, frozen $\gamma_{\cos}\!=\!0.397$) and chr22 (calibration). Splice donor and acceptor (blue) sit $\sim 2$ layers below the intron baseline (red dashed). The asterisk on ``5$'$~UTR$^*$'' marks the entropy-confounded context discussed in \S\ref{sec:discussion}; it is reported for completeness but excluded from the headline hierarchy. Right of each bar: Cohen's $d$ versus the intron baseline with significance label (Mann--Whitney; $\,*$, $**$, $***$, $****$ at $0.05$, $0.01$, $10^{-3}$, $10^{-4}$). \textbf{(b)} Cohen's $d$ versus a chr22 per-position background for splice donor (reference; $-0.433$), ENCODE cCRE-ELS ($-0.190$), GTEx eQTL ($-0.022$) and GWAS Catalog ($-0.040$). The cCRE-ELS effect is measurable; SNP-level eQTL/GWAS annotations are directionally consistent but biologically marginal.}
...
For variants, we use the layer at which $|\DDcos|$ peaks as a depth probe: it asks where the residual trajectory changes most between reference and alternate sequence. This is not a clinical scorer claim. We test this on a ClinVar~\cite{landrum2020clinvar} 15-cancer-gene cohort with cached SNV features and a newly forwarded indel panel. Variants are class-stratified by ClinVar molecular-consequence (\textsc{mc}) priority, in decreasing order: canonical-splice, nonsense, frameshift, missense, synonymous, intron.

Argmax-layer medians order \emph{intron $<$ frameshift $<$ nonsense $<$ missense $\approx$ canonical splice $<$ synonymous} (Fig.~\ref{fig:variants}), differing across the six classes at Kruskal--Wallis $p\!=\!3.0\!\times\!10^{-10}$~\cite{kruskal1952wallis}. Adjacent-pair Dunn--Bonferroni tests~\cite{mann1947whitney,benjamini1995fdr} highlight the largest jump as nonsense $\to$ missense ($p_{\mathrm{adj}}\!<\!10^{-3}$); the remaining adjacent gaps fall within noise at these sample sizes ($n\!=\!32$--$1{,}740$). Thus the strongest supported claim is a population-level shift from early-truncating consequences toward deeper missense/splice disruptions, not clean separability of every adjacent class.

\begin{figure}[!t]
\centering
\includegraphics[width=0.98\columnwidth]{figures/fig_variants.png}
...
As a sanity check on the trajectory's information content, a logistic regression on the full 32-dimensional $\DDcos$ trajectory reaches 10-fold stratified AUROC $0.844$ on the $8{,}008$ ClinVar P/LP-vs-B/LB cohort, against $0.729$ for the best single-layer tap; full per-feature numbers, DeLong~\cite{delong1988comparing} pairwise tests, the per-layer ablation, and the four-panel ROC/DeLong/per-gene figure are deferred to App.~\ref{app:auroc-detail} (Tables~\ref{tab:auroc}, \ref{tab:per-layer}; Fig.~\ref{fig:auroc}). We do not benchmark \gDTR{} against AlphaMissense~\cite{cheng2023alphamissense}, SpliceAI~\cite{jaganathan2019spliceai}, or CADD~\cite{rentzsch2019cadd} in the main text because the contribution here is a layer-resolved interpretability axis, not a replacement variant scorer.


\subsection{Robustness and tokenisation limits}
\label{sec:cross-arch}

Extending the chr22 windows to four genomic foundation models (Evo~2~7B, HyenaDNA-large, NT-v2~500M, DNABERT-2) gives a bounded robustness result (Table~\ref{tab:cross-arch-summary}; per-context bars in Fig.~\ref{fig:cross-arch-context} and the two-tier rank-concordance summary in Fig.~\ref{fig:cross-arch}, App.~\ref{app:cross-arch}). The donor $<$ intron inequality replicates in the two per-bp causal LMs at depth-normalised scale, and their per-window settling ranks correlate moderately (Spearman $\rho\!=\!+0.516$). The two MLMs operate at $k$-mer/BPE token granularity, so their per-window readout cannot directly test a single-base splice junction. We therefore interpret the cross-model analysis as evidence for replication within per-bp causal LMs and as a diagnosis of tokenisation limits for MLMs.
...
Four caveats temper the interpretation. \emph{(i)} Settling depth is an internal correlational readout, not proof of a causal circuit. \emph{(ii)} At $5'$~UTR, $c(t)$ partially confounds with prediction entropy ($\rho\!=\!+0.41$ vs.\ $|\rho|\!\le\!0.15$ elsewhere), so we do not promote 5$'$~UTR as part of the headline hierarchy. \emph{(iii)} We control for next-token entropy but not yet for $k$-mer rarity, GC content, or dinucleotide composition; matched-composition controls are the most important next robustness check. \emph{(iv)} The cross-architecture comparison is limited by readout granularity: per-bp causal LMs can test splice positions directly, whereas $k$-mer/BPE MLMs yield only per-window estimates. Adding Caduceus~\cite{schiff2024caduceus} as a second per-position causal LM would help disentangle architecture from tokenisation.
...
\gDTR{} establishes a training-free, layer-resolved interpretability axis for genomic foundation models. Per-token settling depth shows that splice sites and enhancer-like cCREs stabilise earlier than intronic or coding contexts, and the splice signal is not explained by next-token entropy. Targeted motif/flank perturbations show why this depth should be interpreted carefully: the central GT motif contributes a small detection signal, whereas real flanking grammar requires deeper integration. Variant-induced $\DDcos$ trajectories add a second use case, revealing consequence-specific shifts in the layer where residual disruption peaks. Whole-genome scaling, matched-composition controls, and integration with sparse-dictionary~\cite{cunningham2023sparse} and causal-edit~\cite{meng2022rome} methods are left to future work.
...
windows) yields the values in Table~\ref{tab:idle-block}. Two facts
...
$12{,}978$-window panel; numbers are produced by the
\texttt{results/phase4/per\_model\_summary.json} pipeline.
...
Underlying numbers: \texttt{results/phase4/per\_model\_summary.json}.}
...
gap. The full 32-row table is released as
\texttt{per\_layer\_auroc.csv}.
...
Table~\ref{tab:auroc}; the DeLong paired comparisons (b) show that
...
features. \textbf{(b)}~DeLong paired comparisons: $\Delta D$ adds
...
the 32-d vector beats it by $+0.05$ to $+0.12$.
...
reveals an unexpected ordering: \emph{non-canonical} splice donors
(AT-AC and other rare motifs, pooled $\bar c\!=\!25.13$,
...
($\bar c\!=\!27.01$, $n\!=\!5{,}165$). The shallower-than-intron pattern
