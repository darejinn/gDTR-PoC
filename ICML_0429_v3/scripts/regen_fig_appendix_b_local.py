"""Local regeneration of Fig 4 (per-context bars across 4 FMs).
...
def main() -> None:
    ...
    for ax, model in zip(axes, ["evo2", "hyenadna", "nt_v2", "dnabert2"]):
        ...
        ax.set_title(TITLES[model], loc="left", fontsize=9.2,
                     fontweight="bold")
    ...
    fig.tight_layout()
    fig.savefig(OUT_DIR / "fig_appendix_b_local.png", dpi=300)
    fig.savefig(OUT_DIR / "fig_appendix_b_local.pdf")
    plt.close(fig)
    print(f"wrote {OUT_DIR / 'fig_appendix_b_local.png'}")
    print(f"wrote {OUT_DIR / 'fig_appendix_b_local.pdf'}")
