...
Output: writes ``figures/fig_shallowness.{png,pdf}`` next to the v3 LaTeX.
"""
...
def fig_shallowness() -> None:
    ...
    # fig_shallowness.png and the V3_REWRITE_NOTES.md).
    panel_a = [
    ...
    fig.savefig(OUT_DIR / "fig_shallowness.png", dpi=300)
    fig.savefig(OUT_DIR / "fig_shallowness.pdf")
    plt.close(fig)
...
def main() -> None:
    setup_style()
    fig_shallowness()
    print(f"wrote {OUT_DIR/'fig_shallowness.png'}")
    print(f"wrote {OUT_DIR/'fig_shallowness.pdf'}")
