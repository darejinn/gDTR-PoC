...
def panel_auroc_summary(ax, base) -> None:
    ...
    for i, method in enumerate(METHOD_ORDER):
        ...
        ax.errorbar(lg["mean_auroc"], y[i] + width / 2,
                    ...
                    fmt="none", ecolor="black", lw=0.7, capsize=2)
        ax.text(s["mean_auroc"] + 0.01, y[i] - width / 2,
                f"{s['mean_auroc']:.3f}", va="center", fontsize=7.5)
        ax.text(lg["mean_auroc"] + 0.01, y[i] + width / 2,
    ...
    ax.set_title("(a) Method comparison", loc="left", fontweight="bold")
...
def panel_delong(ax, base) -> None:
    ...
    ax.barh(y, deltas, color=colors, edgecolor="black", lw=0.5, height=0.6)
    for yi, (d, p) in enumerate(zip(deltas, pvals)):
        ax.text(d + (0.005 if d > 0 else -0.005), yi,
                f"$\\Delta$={d:+.3f}\n$p$={p:.0e}",
                ha="left" if d > 0 else "right", va="center", fontsize=7.5)
    ax.axvline(0, color="black", lw=0.6)
    ax.set_yticks(y)
    ax.set_yticklabels(labels)
    ax.invert_yaxis()
    ax.set_xlabel("$\\Delta$AUROC (DeLong paired)")
    ax.set_title("(b) Pairwise DeLong tests", loc="left", fontweight="bold")
...
def panel_per_layer(ax) -> None:
    ...
    ax.set_title("(c) Per-layer ablation", loc="left", fontweight="bold")
...
def panel_logo(ax, base) -> None:
    ...
    ax.set_title("(d) Leave-one-gene-out", loc="left", fontweight="bold")
    ax.legend(frameon=False, loc="lower right", fontsize=7.5)
...
def main() -> None:
    ...
    fig.tight_layout()
    fig.savefig(OUT_DIR / "fig_auroc_local.png", dpi=300)
    fig.savefig(OUT_DIR / "fig_auroc_local.pdf")
    plt.close(fig)
    print(f"wrote {OUT_DIR / 'fig_auroc_local.png'}")
    print(f"wrote {OUT_DIR / 'fig_auroc_local.pdf'}")
