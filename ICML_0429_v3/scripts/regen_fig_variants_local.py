...
Output: `figures/fig_variants_local.{png,pdf}` (renamed so it does not
...
def main() -> None:
    ...
    fig.tight_layout()
    fig.savefig(OUT_DIR / "fig_variants_local.png", dpi=300)
    fig.savefig(OUT_DIR / "fig_variants_local.pdf")
    plt.close(fig)
    print(f"wrote {OUT_DIR / 'fig_variants_local.png'}")
    print(f"wrote {OUT_DIR / 'fig_variants_local.pdf'}")
