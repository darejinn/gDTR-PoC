...
\icmltitlerunning{gDTR: Layer-wise Settling Depth for Genomic Foundation Models}
...
We introduce a fourth, hierarchical axis. The Deep-Thinking Ratio of~\citet{chen2026deepthinking} measures, in NLP transformers, the layer at which intermediate logit-lens distributions~\cite{nostalgebraist2020logitlens,belrose2023tunedlens,pal2023futurelens} converge within $\varepsilon$ of the final layer; under the iterative-refinement view of transformer prediction~\cite{geva2022ffn,meng2022rome}, this is a domain-agnostic readout of residual-stream commitment depth. \gDTR{} measures the same latent quantity on a different input domain -- a single nucleotide token -- without importing the cognitive ``thinking'' connotation. Three adaptations are required: a cosine unembed-residual lens to escape JSD saturation in the $|V|\!\le\!512$ vocabulary; a running-minimum envelope to tame architecture-specific non-monotonicity (StripedHyena~\cite{poli2023striped}, pure Hyena~\cite{poli2023hyena}, Transformer-MLM~\cite{vaswani2017attention}); and calibration against the post-final-norm reference to handle Evo~2's idle final block (\S\ref{sec:framework}). Fig.~\ref{fig:schematic} summarises the readout. The remainder of this paper tests a two-tier hypothesis on Evo~2~7B and three peer genomic foundation models: \emph{(i)} biologically salient sequence elements stabilise at characteristically shallow layers (depth-of-recognition); \emph{(ii)} the within-recognised-class depth ordering is determined by flanking-grammar complexity rather than by motif intrinsic strength.
...
\begin{equation}
  ...
  \label{eq:cos-lens}
\end{equation}
where $\hnorm(t)$ is the post-final-norm output of the model after the canonical interpretively-distinct tap at $L^\star\!=\!29$ has been propagated through the final RMSNorm. We define the \emph{settling depth} via a running-minimum envelope,
\begin{equation}
  c(t) \;=\; \min\bigl\{\, \ell \;:\; \runmin\,\Dcos(\ell, t) \le \gamma_{\cos}\bigr\},
  \label{eq:settling}
\end{equation}
with $\runmin\,\Dcos(\ell,t) = \min_{k\le\ell}\Dcos(k,t)$. NLP transformers exhibit near-monotonic logit-lens convergence~\cite{nostalgebraist2020logitlens,belrose2023tunedlens}; genomic CLMs do not (Hyena alignment rotations, Evo~2 idle final block) and the running-min collapses the resulting non-monotonicity into a monotone-non-increasing trace. The threshold $\gamma_{\cos}$ is set by \emph{regional $q_{70}$ calibration}: the 70th percentile of the running-min at the penultimate layer over the analysis region. The 70th percentile sits inside a flat plateau of a $5\!\times\!5$ grid sweep over $(\gamma_{\cos},\rho)$ (App.~\ref{app:gamma-sweep}; SD across 25 cells $<0.04$). The original deep-thinking ratio $\mathrm{DTR}(t)\!=\!\mathbf{1}\{c(t) \!>\! \rho L\}$ follows with $\rho\!=\!0.85$.

\paragraph{Evo 2's idle final block.} On chr22 sanity sequences, Evo~2's last block is architecturally idle: $\max_{t}|h_{30}(t)\!-\!h_{31}(t)|\!=\!0$ exactly (residual passthrough), and only at block $29$ does $\cos(h_{\ell},\hnorm)$ become interpretively distinct from $\hnorm$ (sign-flip to $-0.013$). The canonical NLP convention of ``tap the last block'' therefore breaks; we lock the canonical tap at $L^\star\!=\!29$ and use $\hnorm$ as the reference (full evidence in App.~\ref{app:architectural-quirk}). This handling is itself a method contribution for subsequent Evo~2 interpretability studies.

\paragraph{Tuned-lens defence against norm-induced rotation.} A per-layer tuned-lens affine fit~\cite{belrose2023tunedlens} reaches $\ge 98\%$ MSE recovery on $30/32$ Evo~2 layers (canonical $L^\star\!=\!29$: $0.9996$; App.~\ref{app:tuned-lens}). Linear recoverability of every interior tap to the post-norm frame implies that $\Dcos$ tracks information-content stabilisation rather than alignment with a norm-rotated frame. The framework remains training-free: no affine weights are used at inference.
...
\textbf{An ordered hierarchy.} On the seven canonical genomic contexts (Fig.~\ref{fig:shallowness}a), splice donor and acceptor sites converge $\sim 2$ layers earlier than the intronic baseline, while non-splice contexts cluster at or above the baseline. We report Cohen's $d$ as the primary effect-size measure throughout; Mann--Whitney $p$-values reach $<\!10^{-4}$ at all contexts but are inflated by the very large $n$ ($\sim\!10^7$ pooled positions) and should be read as a sanity rather than a magnitude statement. The shallowness axis extends to ENCODE cCRE-ELS enhancer-like elements~\cite{moore2020encode}: on chr22, cCRE-ELS positions are shallower than a per-position background by Cohen's $d\!=\!-0.190$, against a splice-donor reference of $-0.433$ (Fig.~\ref{fig:shallowness}b). The full hierarchy reads \emph{splice donor $<$ cCRE-ELS $<$ intron $<$ coding exon $<$ 5$'$~UTR} (Fig.~\ref{fig:schematic}c). GTEx eQTLs~\cite{gtex2020} ($d\!=\!-0.022$) and GWAS Catalog SNPs~\cite{sollis2023gwas} ($d\!=\!-0.040$) sit on the same axis at biologically marginal effect sizes despite formal significance.
...
\textbf{Detection vs.\ grammar integration.} Settling depth tracks \emph{recognition} -- whether the model has begun to commit to the token's identity -- but the depth ordering \emph{within} a recognised class is determined by flanking-grammar complexity, not by the intrinsic strength of the central motif. We dissociate the two on $1{,}000$ canonical GT-AG donors (chr22, $\pm 10$~bp pad-averaged $c$). Real donors settle at $\bar c\!=\!26.77$. (i) Replacing the central GT with AA, while preserving the entire $\pm 100$~bp flank, deepens $\bar c$ by $0.46$ layers (Cohen's $d\!=\!-0.09$, paired Wilcoxon $p\!=\!2.3\!\times\!10^{-32}$): a small but highly reliable \emph{detection} signal carried by the canonical dinucleotide. (ii) Dinucleotide-shuffling the $\pm 100$~bp flank while preserving the central GT \emph{shallows} $\bar c$ by $3.18$ layers ($d\!=\!+0.51$, $p\!=\!4.1\!\times\!10^{-59}$): real flanking grammar engages deeper integration, while a shuffled flank lets the GT pop out in isolation. The same logic explains the within-splice motif breakdown (App.~\ref{app:splice-canonical}, where non-canonical donors converge \emph{earlier} than canonical GC-AG): GC-AG carries a more constrained branch-point / polypyrimidine grammar that takes deeper integration. \emph{Settling depth therefore measures detection plus grammar integration jointly; it is not a meter of motif intrinsic strength.}
...
As a sanity check on the trajectory's information content (and not as a scorer claim), a logistic regression on the full 32-dimensional $\DDcos$ trajectory reaches 10-fold stratified AUROC $0.844$ on the $8{,}008$ ClinVar P/LP-vs-B/LB cohort, against $0.729$ for the best single-layer tap; full per-feature numbers, DeLong~\cite{delong1988comparing} pairwise tests, and the per-layer ablation are deferred to App.~\ref{app:auroc-detail} and App.~\ref{app:per-layer}. We deliberately do \emph{not} benchmark \gDTR{} against AlphaMissense~\cite{cheng2023alphamissense}, SpliceAI~\cite{jaganathan2019spliceai} or CADD~\cite{rentzsch2019cadd}: \gDTR{} is positioned as an interpretability axis rather than a variant scorer, and a head-to-head AUROC table belongs in a scorer companion paper.


\subsection{Architecture dependence as a robustness check}
\label{sec:cross-arch}

Extending the chr22 windows to four genomic foundation models with divergent backbones (Evo~2~7B, HyenaDNA-large, NT-v2~500M, DNABERT-2) reveals not a universal invariance but a tokenisation-gated, two-tier structure (Tab.~\ref{tab:cross-arch-summary}; App.~\ref{app:cross-arch}). Per-position causal-LMs replicate the donor $<$ intron inequality at depth-normalised scale (within-family Spearman $\rho\!=\!+0.516$, moderate given layer counts of 32 vs 8). Per-window MLMs average splice-junction signal across BPE/$k$-mer tokens. \emph{Tokenisation granularity}, not architecture per se, gates the readout.
...
\begin{tabular}{@{}lcc@{}}
\toprule
Family & Per-pos. & Donor$\,{<}\,$intron \\
\midrule
Causal LM (Evo~2, HyenaDNA) & yes      & yes \\
MLM (NT-v2, DNABERT-2)      & per-win. & confounded \\
\bottomrule
\end{tabular}
\end{table}
...
\gDTR{} establishes a training-free, layer-resolved interpretability axis for genomic foundation models. Per-token settling depth surfaces a coherent biological hierarchy (splice donor $<$ cCRE-ELS $<$ intron $<$ coding exon $<$ 5$'$~UTR), survives partial-correlation against prediction entropy, and dissociates motif \emph{detection} from \emph{grammar integration}: canonical GT motif contributes $0.46$ layers, flanking $\pm 100$~bp contributes $3.18$. The same readout class-stratifies $4{,}023$ ClinVar P/LP variants by their disruption layer ($p\!=\!3\!\times\!10^{-10}$). Whole-genome scaling and integration with sparse-dictionary~\cite{cunningham2023sparse} and causal-edit~\cite{meng2022rome} methods are left to future work.
