...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
Biologically salient sequence elements stabilize at characteristically shallow layers in genomic causal language models, and this depth ordering tracks the lexical strength of the underlying biological motif. We introduce \gDTR{} (\emph{Genomic Deep-Thinking Ratio}), a training-free, layer-resolved framework that measures, for every nucleotide token, the \emph{settling depth} $c(t)$ -- the layer at which intermediate predictions stabilise on the final-layer distribution -- via a cosine unembed-residual lens with running-minimum convergence. On Evo~2~7B, $c(t)$ recovers a coherent biological hierarchy (splice donor $<$ ENCODE cCRE-ELS $<$ exon $<$ intron $<$ 5$'$ UTR), survives partial-correlation control against next-token prediction entropy, and shifts by $0.46$ layers when the canonical GT motif is destroyed (paired $p\!\ll\!10^{-30}$). The same readout yields class-stratified disruption depths across $4{,}023$ ClinVar P/LP variants ($p\!=\!3\!\times\!10^{-10}$). \gDTR{} thus offers a fourth, hierarchical interpretability axis -- \emph{where, in the layer stack, biology is recognised} -- complementary to attention-, dictionary-, and output-side tools.
\end{abstract}
